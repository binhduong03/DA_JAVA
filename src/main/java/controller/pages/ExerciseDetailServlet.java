...
public class ExerciseDetailServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		request.setAttribute("welcome", "/View/Pages/Exercise/exercise-detail.jsp");
		String exerciseId = request.getParameter("id");
        int exercise_id = Integer.parseInt(exerciseId);
		...
		List<Exercise> allexercise = e.getExerciseCourseById(course_id);
		List<Lecture> lecture = ld.getLectureCourseById(course_id);
		ChapterDAO c = new ChapterDAO();
		List<Chapter> chapter = c.getChapterCourseById(course_id);
        Exercise exercise = e.getExerciseById(exercise_id);
        request.setAttribute("course", course);
        request.setAttribute("chapter", chapter);
        request.setAttribute("lecture", lecture);
        request.setAttribute("allexercise", allexercise);
